...
NodeType BTreeNode::type() const {
    return static_cast<NodeType>(read_node_header().node_type);
}

uint16_t BTreeNode::num_keys() const {
    return read_node_header().num_keys;
}

BTreeNode::PageId BTreeNode::link() const {
    return read_node_header().link;
}
...
size_t BTreeNode::free_space() const {
    ...
    if (data_start < slot_end) return 0;
...
bool BTreeNode::leaf_remove(std::string_view key) {
    NodeHeader h = read_node_header();
    uint16_t i = lower_bound_leaf(key);
    if (i >= h.num_keys || leaf_entry(i).key != key) return false;
...
std::optional<std::string> BTreeNode::leaf_find(std::string_view key) const {
    uint16_t i = lower_bound_leaf(key);
    if (i >= num_keys()) return std::nullopt;
    auto e = leaf_entry(i);
