...
std::optional<std::string> BTree::get(std::string_view key) const {
    ...
    for (;;) {
        ...
        fm_.read_page(cur, p);
        BTreeNode node(p);
        if (node.is_leaf()) return node.leaf_find(key);
        cur = node.internal_find_child(key);
    }
}
...
bool BTree::remove(std::string_view key) {
    ...
    for (;;) {
        ...
        BTreeNode node(p);
        if (node.is_leaf()) {
            if (!node.leaf_remove(key)) return false;
            fm_.write_page(p);
            return true;
        }
        cur = node.internal_find_child(key);
...
std::vector<std::pair<std::string, std::string>> BTree::range(
