...
void Wal::append(OpType op, std::string_view key, std::string_view value) {
    ...
    // Bump valid_end_ if we have already replayed (so size_bytes() is meaningful).
    if (replayed_) {
