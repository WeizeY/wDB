...
void FileManager::read_meta() {
  ...
  if (mp.version != kVersion) {
    throw std::runtime_error("unsupported wDB version");
  }
  num_pages_ = mp.num_pages;
}

void FileManager::write_meta_num_pages() {
  Page meta;
...
Page::PageId FileManager::allocate_page(PageType type) {
  ...
  p.init(id, type);
  write_page(p);
  write_meta_num_pages();
  return id;
}

void FileManager::read_page(Page::PageId id, Page &page) const {
  if (id >= num_pages_) {
    throw std::out_of_range("read_page: page_id " + std::to_string(id) +
  ...
  if (!page.verify_checksum()) {
    throw std::runtime_error("page " + std::to_string(id) +
                             " checksum mismatch");
  }
}

void FileManager::write_page(const Page &page) {
  // Local copy so checksum stamp does not mutate caller.
  Page copy = page;
  ...
  if (n != static_cast<ssize_t>(kPageSize)) {
    throw std::runtime_error("pwrite failed for page " +
                             std::to_string(copy.header().page_id));
  }
}
